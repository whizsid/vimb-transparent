...
static void on_webctx_init_web_extension(WebKitWebContext *webctx,
                                         gpointer data);
...
static void on_webdownload_failed(WebKitDownload *download, GError *error,
                                  Client *c);
...
static void on_counted_matches(WebKitFindController *finder, guint count,
                               Client *c);
...
gboolean vb_download_set_destination(Client *c, WebKitDownload *download,
                                     ...
                                     const char *path) {
  char *download_path, *dir, *file, *uri, *basename = NULL, *decoded_uri = NULL;
 ...
 */
void vb_echo_force(Client *c, MessageType type, gboolean hide,
                   const char *error, ...) {
  char *buffer;
  va_list args;
 ...
 */
void vb_enter_prompt(Client *c, char id, const char *prompt,
                     gboolean print_prompt) {
...
void vb_input_update_style(Client *c) {
  ...
  if (type == MSG_ERROR) {
    gtk_style_context_add_class(gtk_widget_get_style_context(c->input),
                                "error");
  } else {
    gtk_style_context_remove_class(gtk_widget_get_style_context(c->input),
                                   "error");
  }
}
...
void vb_mode_add(char id, ModeTransitionFunc enter, ModeTransitionFunc leave,
  ...
  if (!vb.modes) {
    vb.modes = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL,
                                     (GDestroyNotify)mode_free);
  }
...
VbResult vb_mode_handle_key(Client *c, int key) {
  ...
  if (c->mode && c->mode->keypress) {
    ...
    if (c->mode) {
      PRINT_DEBUG("%c[%d]: %#.2x '%c' -> %c[%d]", id - ' ', flags, key,
                  (key >= 0x20 && key <= 0x7e) ? key : ' ', c->mode->id - ' ',
                  c->mode->flags);
    }
#else
    res = c->mode->keypress(c, key);
...
static void statusbar_update_downloads(Client *c, GString *status) {
  ...
  if (c->state.downloads) {
    ...
    g_string_append_printf(status, " %d %s (ETA %us)", list_length,
                           list_length == 1 ? "dnld" : "dnlds", remaining_max);
  }
}
...
static Client *client_new(WebKitWebView *webview) {
  ...
  g_signal_connect(c->finder, "counted-matches", G_CALLBACK(on_counted_matches),
                   c);
...
static void client_show(WebKitWebView *webview, Client *c) {
  ...
  gtk_box_pack_start(GTK_BOX(box), GTK_WIDGET(c->statusbar.box), FALSE, FALSE,
                     0);
  gtk_box_pack_end(GTK_BOX(box), GTK_WIDGET(c->input), FALSE, FALSE, 0);

  /* Set the default style for statusbar and inputbox. */
  gtk_style_context_add_provider(
      gtk_widget_get_style_context(GTK_WIDGET(c->statusbar.box)),
  ...
  char *wid;
  wid = g_strdup_printf("%d",
                        (int)GDK_WINDOW_XID(gtk_widget_get_window(c->window)));
...
static GtkWidget *create_window(Client *c) {
  ...
  g_object_connect(
      G_OBJECT(window), "signal::destroy", G_CALLBACK(on_window_destroy), c,
      "signal::delete-event", G_CALLBACK(on_window_delete_event), c,
      "signal::key-press-event", G_CALLBACK(on_map_keypress), c, NULL);
...
static void input_print(Client *c, MessageType type, gboolean hide,
  ...
  if (hide) {
    ...
    c->state.input_timer =
        g_timeout_add_seconds(MESSAGE_TIMEOUT, (GSourceFunc)input_clear, c);
 ...
 * modes hashmap.
 */
static void mode_free(Mode *mode) { g_slice_free(Mode, mode); }
...
static void spawn_new_instance(const char *uri) {
  ...
  /* memory allocation */
  char **cmd =
      g_malloc_n(3 /* basename + uri + ending NULL */
                     + (vb.configfile ? 2 : 0)
#ifndef FEATURE_NO_XEMBED
                     + (vb.embed ? 2 : 0)
#endif
                     + (vb.incognito ? 1 : 0) + (vb.profile ? 2 : 0) +
                     (vb.no_maximize ? 1 : 0) + g_slist_length(vb.cmdargs) * 2,
                 sizeof(char *));
...
static void on_webctx_download_started(WebKitWebContext *webctx,
  ...
  if (GET_BOOL(c, "download-use-external")) {
    ...
    g_signal_connect(download, "finished", G_CALLBACK(on_webdownload_finished),
                     c);
...
static void spawn_download_command(Client *c, WebKitURIResponse *response) {
  ...
  if (!g_shell_parse_argv(cmd, &argc, &argv, &error)) {
    g_warning("Could not parse download-command '%s': %s", cmd, error->message);
    g_error_free(error);
    g_free(cmd);
    return;
  }
...
static void on_webdownload_failed(WebKitDownload *download, GError *error,
  ...
  if (basename) {
    vb_echo(c, MSG_ERROR, FALSE, "Download of %s failed (%s)", basename,
            error->message);
    g_free(basename);
  }
}
...
static WebKitWebView *on_webview_create(WebKitWebView *webview,
  ...
  if (c->config.prevent_newwindow) {
    ...
    vb_load_uri(
        c, &(Arg){TARGET_CURRENT, (char *)webkit_uri_request_get_uri(req)});
...
static void decide_navigation_action(Client *c, WebKitPolicyDecision *dec) {
  ...
  if ((c->mode->flags & FLAG_NEW_WIN) ||
    ...
    if (webkit_navigation_action_get_navigation_type(a) ==
            WEBKIT_NAVIGATION_TYPE_LINK_CLICKED &&
        button == 1 && mod & GDK_SHIFT_MASK && strcmp(uri, "about:blank")) {
...
static void decide_response(Client *c, WebKitPolicyDecision *dec) {
  ...
  res = webkit_response_policy_decision_get_response(
...
static void on_webview_notify_estimated_load_progress(WebKitWebView *webview,
                                                      ...
                                                      Client *c) {
  c->state.progress =
      webkit_web_view_get_estimated_load_progress(webview) * 100;
  vb_statusbar_update(c);
  update_title(c);
}
...
static gboolean on_webview_web_process_crashed(WebKitWebView *webview,
                                               Client *c) {
  vb_echo(c, MSG_ERROR, FALSE, "Webview Crashed on %s",
...
static void update_title(Client *c) {
  ...
  /* Show load status of page or the downloads. */
  if (c->state.progress != 100) {
    char *title = g_strdup_printf("[%i%%] %s", c->state.progress,
...
static void vimb_setup(void) {
  ...
  vb.storage[STORAGE_HISTORY] =
      file_storage_new(dataPath, "history", vb.incognito);
  vb.storage[STORAGE_COMMAND] =
      file_storage_new(dataPath, "command", vb.incognito);
  vb.storage[STORAGE_SEARCH] =
      file_storage_new(dataPath, "search", vb.incognito);
  g_free(dataPath);
  ...
  if (vb.incognito) {
    ...
    manager = webkit_website_data_manager_new(
        "base-data-directory", util_get_data_dir(), "base-cache-directory",
        util_get_cache_dir(), NULL);
  }
  ...
  webkit_web_context_set_process_model(
      vb.webcontext, WEBKIT_PROCESS_MODEL_MULTIPLE_SECONDARY_PROCESSES);
  webkit_web_context_set_cache_model(vb.webcontext,
                                     WEBKIT_CACHE_MODEL_WEB_BROWSER);

  g_signal_connect(vb.webcontext, "initialize-web-extensions",
                   G_CALLBACK(on_webctx_init_web_extension), NULL);

  /* Add cookie support only if the cookie file exists and web context is
   * not ephemeral */
  if (vb.files[FILES_COOKIE] &&
      !webkit_web_context_is_ephemeral(vb.webcontext)) {
    WebKitCookieManager *cm;

    cm = webkit_web_context_get_cookie_manager(vb.webcontext);
    webkit_cookie_manager_set_persistent_storage(
        cm, vb.files[FILES_COOKIE], WEBKIT_COOKIE_PERSISTENT_STORAGE_SQLITE);
  }
 ...
 */
void vb_gui_style_update(Client *c, const char *setting_name_new,
                         const char *setting_value_new) {
  g_assert(c);
  ...
  for (i = 0; setting_style_map[i][0]; i++) {
     ...
     * append the css string based on the current config setting. */
      ...
      Setting *setting_value =
          (Setting *)g_hash_table_lookup(c->config.settings, setting_name);
      ...
      if (strlen(setting_value->value.s)) {
        g_string_append_printf(style_sheet, style_string,
                               setting_value->value.s);
      }
    }
  }
...
static WebKitWebView *webview_new(Client *c, WebKitWebView *webview) {
  ...
  if (webview) {
    new = WEBKIT_WEB_VIEW(g_object_new(WEBKIT_TYPE_WEB_VIEW,
                                       ...
                                       "related-view", webview, NULL));
  } else {
    new = WEBKIT_WEB_VIEW(g_object_new(WEBKIT_TYPE_WEB_VIEW,
                                       "user-content-manager", ucm,
                                       "web-context", vb.webcontext, NULL));
  }
  ...
  g_object_connect(
      G_OBJECT(new), "signal::close", G_CALLBACK(on_webview_close), c,
      ...
      G_CALLBACK(on_webview_mouse_target_changed), c,
      "signal::notify::estimated-load-progress",
      G_CALLBACK(on_webview_notify_estimated_load_progress), c,
      ...
      "signal::scroll-event", G_CALLBACK(on_scroll), c, "signal::ready-to-show",
      G_CALLBACK(on_webview_ready_to_show), c, "signal::web-process-crashed",
      G_CALLBACK(on_webview_web_process_crashed), c, "signal::authenticate",
      G_CALLBACK(on_webview_authenticate), c, "signal::enter-fullscreen",
      G_CALLBACK(on_webview_enter_fullscreen), c, "signal::leave-fullscreen",
      G_CALLBACK(on_webview_leave_fullscreen), c, NULL);
  ...
  g_signal_connect(ucm, "script-message-received::focus",
                   G_CALLBACK(on_script_message_focus), NULL);
...
void preference_apply(Preference *preference, void *args) {
  Client *client = args;
  GdkRGBA *ssl_color = malloc(sizeof(GdkRGBA));
  GdkRGBA *unsecure_color = malloc(sizeof(GdkRGBA));

  preference->background.alpha = preference->opacity;

  memcpy(ssl_color, &preference->palette[2], sizeof(GdkRGBA));
  ssl_color->alpha = preference->opacity;

  memcpy(unsecure_color, &preference->palette[1], sizeof(GdkRGBA));
  unsecure_color->alpha = preference->opacity;

  char *style_sheet = malloc(sizeof(char) * 1000);
  sprintf(style_sheet,
          "#statusbar {background-color: %s; color: %s; font-family: %s; "
          "font-size: %s;}\n"
          "#statusbar.secure {background-color: %s;}\n"
          "#statusbar.unsecure {background-color: %s;}\n"
          "#input.view {background-color: %s; font-family: %s; font-size: %s;}"
          "#input.view text {background-color: rgba(0,0,0,0); color: %s;}",
          gdk_rgba_to_string(&preference->background),
          gdk_rgba_to_string(&preference->bold), preference->font_family,
          preference->font_size, gdk_rgba_to_string(ssl_color),
          gdk_rgba_to_string(unsecure_color),
          gdk_rgba_to_string(&preference->background), preference->font_family,
          preference->font_size, gdk_rgba_to_string(&preference->foreground));

  gtk_css_provider_load_from_data(vb.style_provider, style_sheet, -1, NULL);

  G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
  gtk_style_context_invalidate(gtk_widget_get_style_context(client->input));
  G_GNUC_END_IGNORE_DEPRECATIONS;

  webkit_web_view_set_background_color(client->webview, &preference->background);
}
...
int main(int argc, char *argv[]) {
  ...
  GOptionEntry opts[] = {
      {"cmd", 'C', 0, G_OPTION_ARG_CALLBACK,
       (GOptionArgFunc *)autocmdOptionArgFunc,
       "Ex command run before first page is loaded", NULL},
      {"config", 'c', 0, G_OPTION_ARG_FILENAME, &vb.configfile,
       "Custom configuration file", NULL},
#ifndef FEATURE_NO_XEMBED
      {"embed", 'e', 0, G_OPTION_ARG_STRING, &winid,
       "Reparents to window specified by xid", NULL},
#endif
      {"incognito", 'i', 0, G_OPTION_ARG_NONE, &vb.incognito,
       "Run with user data read-only", NULL},
      {"profile", 'p', 0, G_OPTION_ARG_CALLBACK,
       (GOptionArgFunc *)profileOptionArgFunc, "Profile name", NULL},
      {"version", 'v', 0, G_OPTION_ARG_NONE, &ver, "Print version", NULL},
      {"no-maximize", 0, 0, G_OPTION_ARG_NONE, &vb.no_maximize,
       "Do no attempt to maximize window", NULL},
      {"bug-info", 0, 0, G_OPTION_ARG_NONE, &buginfo,
       "Print used library versions", NULL},
      {NULL}};
  ...
  if (buginfo) {
    ...
    printf("libsoup compile: %d.%d.%d\n", SOUP_MAJOR_VERSION,
           SOUP_MINOR_VERSION, SOUP_MICRO_VERSION);
  ...
  prefwatch = preference_watch(preference_apply, c);
  gtk_main();
#ifdef FREE_ON_QUIT
  vimb_cleanup();
#endif
  // preference_watch_cancel(prefwatch);
  pthread_exit(NULL);
